 ...
 * but for root.left, root.left.left, root.left.right, root.right.left
 * root.right .right, they are in one level.
 * so for each level, we have to know the uplevel and get the left and right
 * nodes of each nodes in the uplevel
 */
public class LevelOrderTraversal {
    ...
    public void traverseLevelOrder(List<List<Integer>> result, TreeNode root, int level) {
        ...
        // next level: level+1; for left of the root, level is 1 and the result.size()
        // is 1 too,
    ...
    /**
     * Approach 2: Use Queue as helper structure when we have level order traversal
     * BFS(广度优先搜索)
