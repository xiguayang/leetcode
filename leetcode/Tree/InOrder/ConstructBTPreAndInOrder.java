...
public class ConstructBTPreAndInOrder {
  public TreeNode buildTree(int[] preorder, int[] inorder) {
     ...
     * 2. according to the root in #1, inorder can split into left subtree and right
     * subtree
     ...
     * 3. if leftsubtree is empty, make the num preorder[i] as root and recursive #1
     * in rightsubtree
...
class Solution {
  public TreeNode buildTree(int[] preorder, int[] inorder) {
    ...
    // 2. traverse inorder to get the index of root
    // 3. the left part of index is left sub tree, and the right is right sub tree
    // get the size of left tree
    // 4. get the start and end index of both left sub tree and right sub tree
    // 5. build the result BT by recursively find these sub trees

    // we can ignore preEnd
